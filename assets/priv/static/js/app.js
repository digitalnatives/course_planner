...
(function e(t, n, r) {
  function s(o, u) {
    ...
    var _require = require("./modal.js"),
        showModal = _require.showModal,
        hideModals = _require.hideModals;

    function initAttendances() {
      document.addEventListener("click", function (e) {
        if (e.target.classList.contains("show-attendance-modal")) {
          var id = e.target.getAttribute("data-attendance-id");
          showModal(".modal[data-attendance-id=\"" + id + "\"]");
        }

        if (e.target.classList.contains("update-attendance")) {
          var _id = e.target.getAttribute("data-attendance-id");

          var value = document.querySelector(".modal[data-attendance-id='" + _id + "'] option:checked").value;

          if (value) {
            document.querySelector("button[data-attendance-id='" + _id + "'] i").classList.add("attendance-comment__icon--set");
          } else {
            document.querySelector("button[data-attendance-id='" + _id + "'] i").classList.remove("attendance-comment__icon--set");
          }
  ...
  }, { "./modal.js": 5 }], 2: [function (require, module, exports) {
    ...
    function getMonday(currentDate) {
      var now = currentDate ? new Date(currentDate) : new Date();
      var day = now.getDay();
    ...
    function toHours(timeString) {
      return timeString.split(/[:.]/).map(function (value, index) {
        return value * Math.pow(60, -index);
      }).reduce(function (a, b) {
        return a + b;
      });
    }
    ...
    function renderName(user) {
      return [user.name, user.family_name, user.nick_name && "(" + user.nick_name + ")"].filter(function (item) {
        return !!item;
      }).join(" ");
    }
    ...
    function updatePointer() {
      ...
      var pointer = document.querySelector(".calendar__classes-pointer");
    ...
    function renderCalendar(startDate, classes, calendar, displayEvery) {
      ...
      var days = new Array(7).fill(monday).map(function (monday, index) {
        var day = new Date(monday);
        day.setDate(day.getDate() + index);
        return day;
      }).map(function (date) {
        var stack = classes.filter(
        // filter the classes so they will contain the classes of the current day
        function (cl) {
          return cl.date === isoDate(date);
        }).sort().map(
        ...
        function (cl1, _, classes) {
          return Object.assign({}, cl1, {
            edges: classes.filter(function (cl2) {
              return cl1 !== cl2 && (cl1.a < cl2.a && cl2.a < cl1.b || cl1.a < cl2.b && cl2.b < cl1.b || cl2.a < cl1.a && cl1.a < cl2.b || cl2.a < cl1.b && cl1.b < cl2.b || cl1.a == cl2.a || cl1.b == cl2.b);
            }).map(function (cl) {
              return cl.index;
            })
          });
        ...
        */
        ...
        var classesWithComponents = [];

        var componentCounter = 0;

        var _loop = function _loop() {
          // pop the top of the stack
          var _stack$slice3 = stack.slice(-1),
              _stack$slice4 = _slicedToArray(_stack$slice3, 1),
              cl = _stack$slice4[0];

          // separate the rest into connected & not connected arrays


          var _stack$slice$reduce = stack.slice(0, -1).reduce(function (acc, item) {
            return {
              connected: item.edges.includes(cl.index) ? acc.connected.concat([item]) : acc.connected,
              notConnected: item.edges.includes(cl.index) ? acc.notConnected : acc.notConnected.concat([item])
            };
          }, { connected: [], notConnected: [] }),
              connected = _stack$slice$reduce.connected,
              notConnected = _stack$slice$reduce.notConnected;
          ...
          connected = connected.map(function (connectedCl) {
            return Object.assign({}, connectedCl, { component: cl.component });
          });
        ...
        */
        ...
        var everyColoredClasses = classesWithComponents.reduce(
        // separate them by component
        function (components, cl) {
          return Object.assign([], components, _defineProperty({}, cl.component, (components[cl.component] || []).concat([cl])));
        }, []).map(
        // color the components
        function (classes, _) {
          var maxColor = 0;

          stack = classes.slice().sort(function (cl1, cl2) {
            return cl1.edges.length > cl2.edges.length;
          });

          var coloredClasses = [];

          var _loop2 = function _loop2() {
            var _stack$slice = stack.slice(-1),
                _stack$slice2 = _slicedToArray(_stack$slice, 1),
                cl = _stack$slice2[0];

            stack = stack.slice(0, -1);

            var connectedColors = coloredClasses.filter(function (connectedCl) {
              return connectedCl.edges.includes(cl.index);
            }).map(function (connectedCl) {
              return connectedCl.color;
            });
            ...
            cl = Object.assign({}, cl, { color: color });
          ...
          coloredClasses = coloredClasses.map(function (cl) {
            return Object.assign({}, cl, { maxColor: maxColor });
          });
        ...
        return { classes: everyColoredClasses, date: date };
      }).map(
      // calculate dimensions from the hours and colors
      function (day) {
        return Object.assign({}, day, {
          classes: day.classes.map(function (cl) {
            var top = (cl.a - 8) * 50;
            var height = (cl.b - 8) * 50 - top;
            var left = cl.color / (cl.maxColor + 1) * 100;
            var width = 100 / (cl.maxColor + 1);

            return Object.assign({}, cl, { top: top, height: height, left: left, width: width });
          })
        });
      }).map(function (day) {
        return "\n        <div class=\"calendar__day\">\n          <div class=\"calendar__day-header\">\n            " + renderDay(day.date) + ", " + isoDate(day.date) + "\n          </div>\n          <div class=\"calendar__classes\">\n            " + new Array(13).fill(1).map(function (time) {
          return "\n                  <div\n                    class=\"\n                      calendar__classes-hour\n                      " + ([0, 6].includes(day.date.getDay()) ? "calendar__classes-hour--weekend" : "") + "\n                    \"\n                  ></div>\n                ";
        }).join("") + "\n            " + day.classes.map(function (cl, i) {
          return "\n                  <div\n                    class=\"calendar__class\"\n                    style=\"\n                      top: " + cl.top + "px;\n                      height: " + cl.height + "px;\n                      left: " + cl.left + "%;\n                      width: " + cl.width + "%;\n                    \"\n                    id=\"" + isoDate(day.date) + "__" + cl.index + "\"\n                  >\n                    <div class=\"calendar__class-course\">\n                      " + cl.course_name + "\n                    </div>\n                    <div class=\"calendar__class-teachers\">\n                      " + cl.teachers.map(function (teacher) {
            return renderName(teacher);
    ...
    function loadCalendar() {
      var displayEvery = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;

      var calendar = document.querySelector(".calendar__wrapper");

      var startDateParameter = window.location.search.slice(1).split("&").map(function (pairString) {
        return pairString.split("=").map(decodeURIComponent);
      }).filter(function (pair) {
        return pair[0] === "date";
      });

      var startDate = startDateParameter.length && startDateParameter[0][1];
    ...
    */
    ...
    var _require2 = require("./multiselect"),
        initializeMultiselects = _require2.initializeMultiselects;

    initializeMultiselects();

    var _require3 = require("./term.js"),
        initTerms = _require3.initTerms;

    initTerms();

    var _require4 = require("./form.js"),
        initForm = _require4.initForm;

    initForm();

    var _require5 = require("./calendar.js"),
        initCalendar = _require5.initCalendar;

    initCalendar();

    var _require6 = require("./attendances.js"),
        initAttendances = _require6.initAttendances;

    initAttendances();
  }, { "./attendances.js": 1, "./calendar.js": 2, "./form.js": 4, "./multiselect": 6, "./term.js": 7, "material-design-lite": 8, "phoenix_html": 9 }], 4: [function (require, module, exports) {

    function initForm() {
      Array.from(document.querySelectorAll(".form-init input")).forEach(function (input) {
        return input.addEventListener("blur", function (e) {
          return e.target.parentNode.classList.remove("form-init");
        }, { once: true });
      });

      Array.from(document.querySelectorAll(".is-invalid select")).forEach(function (select) {
        return select.addEventListener("focus", function (e) {
          var name = e.target.getAttribute("name");

          var match = name.match(/\[(?:year|month|day|hour|minute)\]$/);

          if (match) {
            var nameBeginning = name.slice(0, match.index);

            Array.from(document.querySelectorAll("select[name^=\"" + nameBeginning + "\"]")).forEach(function (select) {
              return select.parentNode.classList.remove("is-invalid");
            });
          } else {
      ...
      document.addEventListener("click", function (e) {
        if (e.target.classList.contains("attendance-form__button")) {
          var radios = Array.from(e.target.parentNode.querySelectorAll("input[type='radio']"));

          var checkedIndex = radios.findIndex(function (radio) {
    ...
    function hideModals() {
      Array.from(document.querySelectorAll(".modal")).forEach(function (modal) {
        return modal.classList.remove("modal--visible");
      });
    }

    module.exports = { showModal: showModal, hideModals: hideModals };
  }, {}], 6: [function (require, module, exports) {

    function renderTags(select) {
      var multiselectId = select.getAttribute("data-multiselect-id");

      var tagsContainer = document.querySelector(".form-multiselect__tags[data-multiselect-id=\"" + multiselectId + "\"]");

      var tags = Array.from(select.children).map(function (domOption) {
        return {
      ...
      var displayImages = select.getAttribute("data-display-images") === "true";
    ...
    function initializeMultiselects() {
      document.addEventListener("click", function (e) {
        if (e.target.hasAttribute("data-multiselect-add")) {
          var multiselectId = e.target.getAttribute("data-multiselect-id");

          var itemId = document.querySelector(".form-multiselect__dropdown[data-multiselect-id=\"" + multiselectId + "\"] option:checked").getAttribute("value");

          document.querySelector(".form-multiselect__select[data-multiselect-id=\"" + multiselectId + "\"] option[value=\"" + itemId + "\"]").selected = true;

          var multiselect = document.querySelector(".form-multiselect__select[data-multiselect-id=\"" + multiselectId + "\"]");

          emitChange(multiselect);
        } else {
          if (e.target.hasAttribute("data-multiselect-delete")) {
            var _multiselectId = e.target.getAttribute("data-multiselect-id");
            var _itemId = e.target.getAttribute("data-multiselect-item-id");

            document.querySelector(".form-multiselect__select[data-multiselect-id=\"" + _multiselectId + "\"] option[value=\"" + _itemId + "\"]").selected = false;

            var _multiselect = document.querySelector(".form-multiselect__select[data-multiselect-id=\"" + _multiselectId + "\"]");

            emitChange(_multiselect);
          }
        }
      });

      Array.from(document.querySelectorAll(".form-multiselect__select")).forEach(function (select) {
        select.addEventListener("change", function (e) {
          return renderTags(e.target);
        });
        renderTags(select);
      });
    }
    ...
    var _require7 = require("./modal.js"),
        showModal = _require7.showModal,
        hideModals = _require7.hideModals;

    function updateHolidayRow(id) {
      var dateString = [document.querySelector("#" + id + "_date_year option:checked").textContent, document.querySelector("#" + id + "_date_month option:checked").textContent, document.querySelector("#" + id + "_date_day option:checked").textContent].join(" ");

      document.querySelector("td[data-property=\"date\"][data-holiday-id=\"" + id + "\"]").textContent = dateString;

      var description = document.querySelector("#" + id + "_description").value;

      document.querySelector("td[data-property=\"description\"][data-holiday-id=\"" + id + "\"]").textContent = description;
    }

    function removeHoliday(id) {
      var row = document.querySelector("tr[data-holiday-id=\"" + id + "\"]");
      row.parentNode.removeChild(row);

      var hiddenId = document.querySelector("#" + id + "_id");
    ...
    function initTerms() {
      Array.from(document.querySelectorAll(".update-holiday[data-holiday-id]")).forEach(function (button) {
        return updateHolidayRow(button.getAttribute("data-holiday-id"));
      });
      ...
      document.addEventListener("click", function (e) {
        if (e.target.classList.contains("add-form-field")) {
          ...
          container.querySelector(".holiday-table__empty-text").insertAdjacentHTML("beforebegin", newRow);
        ...
        if (e.target.classList.contains("show-holiday-modal")) {
          var id = e.target.getAttribute("data-holiday-id");
  ...
  }, { "./modal.js": 5 }], 8: [function (require, module, exports) {
    /**
