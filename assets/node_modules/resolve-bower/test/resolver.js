...
test('bar', function (t) {
    ...
    resolve('foo', { basedir : dir + '/bar' }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, dir + '/bar/bower_components/foo/index.js');
        t.equal(pkg, undefined);
    });
...
test('without basedir', function (t) {
    ...
    tester(t, function (err, res, pkg){
	  ...
	  t.fail(err);
          ...
          t.equal(res, dir + '/bower_components/mymodule.js');
	}
    });
});
...
test('#25: node modules with the same name as node stdlib modules', function (t) {
    ...
    resolve('punycode', { basedir : resolverDir }, function (err, res, pkg) {
        if (err) t.fail(err);
        t.equal(res, resolverDir + '/bower_components/punycode/index.js');
    });
});
