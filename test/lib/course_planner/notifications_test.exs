defmodule CoursePlanner.NotificationsTest do
  ...
  test "send notification when it's day after" do
    ...
    user1 = insert(:user, %{notified: Timex.shift(today, days: -1), notification_frequency_days: 1})
    insert(:notification, %{user_id: user1.id})

    user2 = insert(:user, %{notified: Timex.shift(today, days: -1), notification_frequency_days: 2})
