defmodule CoursePlanner.Courses.OfferedCourses do
  ...
  def find_all_by_user(user, preload \\ []) do
    user
    |> do_find_all_by_user()
    |> Repo.preload(preload)
  end
  def do_find_all_by_user(%{role: "Coordinator"}) do
    Repo.all(OfferedCourse)
  end
  def do_find_all_by_user(%{role: "Teacher", id: user_id}) do
    Repo.all(from oc in OfferedCourse,
      join: t in assoc(oc, :teachers),
      where: t.id == ^user_id)
  end
  def do_find_all_by_user(%{role: "Student", id: user_id}) do
    Repo.all(from oc in OfferedCourse,
